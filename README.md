...
> ⚠️ 本仓库**刻意不包含签名配置**（`build-profile.json5` 中的 `signingConfigs`），
> 因为其中会包含本机绝对路径与签名口令。直接打开工程会因缺少签名而无法构建。
...
4. 点击 `Apply / OK` —— IDE 会自动往 `build-profile.json5` 写回 `signingConfigs`
   以及 `products[].signingConfig` 字段。
